...
exports.addressToScript = function(addr){
    ...
    if (addr.indexOf(':') !== -1) {
        // Remove the prefix (bitcoincash: or bchtest:)
        addr = addr.split(':')[1];
    }
    
    // If it's a CashAddr (starts with q or p), convert to legacy
    if (addr[0] === 'q' || addr[0] === 'p') {
        // This is a CashAddr format, we need to handle it differently
        // For now, we'll skip validation and use the scriptPubKey directly
        // This is a temporary fix - ideally we'd convert CashAddr to legacy
        console.error('CashAddr format detected - please use legacy address format');
        throw new Error('CashAddr not supported - use legacy address');
    }
