...
var BlockTemplate = module.exports = function BlockTemplate(jobId, rpcData, poolAddressScript, extraNoncePlaceholder, reward, txMessages, recipients){
    ...
    this.serializeHeader = function(merkleRoot, nTime, nonce, version){
        ...
        // Previous block hash - needs to be reversed from RPC format
        var prevHashBuff = util.reverseBuffer(Buffer.from(rpcData.previousblockhash, 'hex'));
        prevHashBuff.copy(header, position);
        position += 32;
        
        // Merkle root - needs to be reversed from hex format
        var merkleRootBuff = util.reverseBuffer(Buffer.from(merkleRoot, 'hex'));
        merkleRootBuff.copy(header, position);
        position += 32;
        
        // Timestamp, bits, and nonce in little-endian
        header.writeUInt32LE(parseInt(nTime, 16), position);
        position += 4;
        
        // Bits needs to be reversed from RPC format
        var bitsBuff = util.reverseBuffer(Buffer.from(rpcData.bits, 'hex'));
        bitsBuff.copy(header, position);
        position += 4;
        
        header.writeUInt32LE(parseInt(nonce, 16), position);
