...
var StratumClient = function(options){
    ...
    /**
     * Handles mining.configure messages.
     * Simplified version that accepts version-rolling without strict BIP 310 negotiation.
     *
    ...
    function handleConfigure(message){
        ...
        // Always accept version-rolling if requested - no strict mask negotiation
        if (extensions.includes("version-rolling")) {
            ...
            console.log('[Stratum] Client ' + (_this.workerName || 'unknown') + 
                       ' enabled version-rolling (no mask negotiation)');
        }
