...
var StratumClient = function(options){
    ...
    function handleMessage(message){
        switch(message.method){
            ...
            case 'mining.extranonce.subscribe':
                // Some miners subscribe to extranonce changes
                sendJson({
                    id: message.id,
                    result: false,  // We don't support extranonce subscription
                    error: null
                });
                break;
            default:
    ...
    function handleConfigure(message){
        // ASICBoost configuration response
        var supported = {};
        ...
        if (message.params && message.params[0]) {
            if (message.params[0].includes("version-rolling")) {
                ...
                supported["version-rolling.mask"] = "1fffe000";  // Standard ASICBoost mask
                supported["version-rolling.min-bit-count"] = 16;
            }
        }
