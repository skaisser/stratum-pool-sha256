...
var StratumClient = function(options){
    ...
    // Store the pool's version mask for BIP 310 negotiation
    ...
    function handleConfigure(message){
        ...
        if (extensions.includes("version-rolling")) {
            // poolVersionMask is already defined in the client scope
            ...
            // Debug log the negotiation
            console.log('[Stratum] Client ' + (_this.workerName || 'unknown') + ' version-rolling negotiation:' +
                       ' pool mask: 0x' + poolVersionMask.toString(16) +
                       ', client requested: ' + (clientRequestedMask || 'none'));
            ...
            if (clientRequestedMask) {
                ...
                if (!isNaN(clientMask)) {
                    if (options.enforcePoolVersionMask) {
                        // For compatibility with rental services, use pool's mask
                        // This allows pools to support miners that request restrictive masks
                        // but still need bit 29 for proper operation
                        negotiatedMask = poolVersionMask;
                        console.log('[Stratum] Enforcing pool mask for compatibility (enforcePoolVersionMask=true)');
                    } else {
                        // Strict BIP 310: intersection of pool and client masks
            ...
            if (bitCount >= clientMinBitCount) {
                ...
                console.log('[Stratum] Client ' + (_this.workerName || 'unknown') + 
                           ' negotiated version mask: 0x' + negotiatedMask.toString(16));
            } else {
