...
var JobManager = module.exports = function JobManager(options){
     ...
     * @param {number} [versionMask] - Client's negotiated version mask
    ...
    this.processShare = function(jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName, version, versionMask){
        ...
        // Version validation with proper ASICBoost support
        ...
        // For ASICBoost, validate version rolling if client negotiated a mask
        if (options.coin.asicboost && versionMask && versionInt !== job.rpcData.version) {
            // Check if rolled bits are within negotiated mask
            var rolledBits = versionInt ^ job.rpcData.version;
            var invalidBits = rolledBits & ~versionMask;
            ...
            if (invalidBits !== 0) {
                console.log('Version rolling outside negotiated mask - version: 0x' + versionInt.toString(16) + 
                           ...
                           ', negotiated mask: 0x' + versionMask.toString(16) +
                           ', invalid bits: 0x' + invalidBits.toString(16));
                return shareError([20, 'version rolling outside negotiated mask']);
            }
            
            // Log successful version rolling
            console.log('Valid version rolling - version: 0x' + versionInt.toString(16) + 
                       ', rolled bits: 0x' + rolledBits.toString(16));
        }
