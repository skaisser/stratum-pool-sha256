...
var JobManager = module.exports = function JobManager(options){
    ...
    this.processShare = function(jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName, version, versionMask){
        ...
        var versionInt;
        if (version) {
            // Version is submitted as little-endian hex string, need to parse it correctly
            var versionBuff = Buffer.from(version, 'hex');
            if (versionBuff.length === 4) {
                versionInt = versionBuff.readUInt32LE(0);
            } else {
                versionInt = parseInt(version, 16);
            }
            
            // Debug logging
            if (!this.loggedVersionParsing) {
                this.loggedVersionParsing = true;
                console.log('Version parsing debug:');
                console.log('  Submitted version hex:', version);
                console.log('  Parsed as LE uint32:', versionInt);
                console.log('  In hex:', versionInt.toString(16));
            }
        } else {
        ...
        if (options.coin.asicboost) {
            ...
            if (versionInt !== job.rpcData.version) {
                ...
                if (invalidBits !== 0) {
                    ...
                    // TEMPORARY: Accept shares with minor mask violations for testing
                    // This helps debug the hash calculation issue
                    if ((invalidBits & 0xFF) === invalidBits) {
                        console.log('WARNING: Accepting share with minor mask violation for testing');
                    } else {
        ...
        // Additional debug for hash calculation
        if (!this.debuggedHashCalc) {
            this.debuggedHashCalc = true;
            console.log('Hash calculation debug:');
            console.log('  Header buffer length:', headerBuffer.length);
            console.log('  Header breakdown:');
            console.log('    Version:', headerBuffer.slice(0, 4).toString('hex'));
            console.log('    PrevHash:', headerBuffer.slice(4, 36).toString('hex'));
            console.log('    MerkleRoot:', headerBuffer.slice(36, 68).toString('hex'));
            console.log('    Time:', headerBuffer.slice(68, 72).toString('hex'));
            console.log('    Bits:', headerBuffer.slice(72, 76).toString('hex'));
            console.log('    Nonce:', headerBuffer.slice(76, 80).toString('hex'));
            console.log('  First hash:', util.sha256(headerBuffer).toString('hex'));
            console.log('  Double hash (sha256d):', headerHash.toString('hex'));
            console.log('  Algorithm:', options.coin.algorithm);
            console.log('  Target from bits:', job.target.toString(16));
        }
        
        // Temporary debug for BCH shares
        if (!this.debuggedBCHShare) {
            this.debuggedBCHShare = true;
            console.log('BCH Share Debug:');
            console.log('  Job ID:', jobId);
            console.log('  Block Height:', job.rpcData.height);
            console.log('  extraNonce1:', extraNonce1);
            console.log('  extraNonce2:', extraNonce2);
            console.log('  nTime:', nTime, '(int:', nTimeInt, ')');
            console.log('  nonce:', nonce);
            console.log('  version:', version || 'none', '(int:', versionInt.toString(16), ')');
            console.log('  headerBuffer:', headerBuffer.toString('hex'));
            console.log('  headerHash:', headerHash.toString('hex'));
            console.log('  Coinbase TX:', coinbaseBuffer.slice(0, 100).toString('hex'), '...');
        }
        ...
        // Debug logging for first share only
        if (!this.debuggedShareCalc) {
            ...
            console.log('  calculated shareDiff:', shareDiff);
