...
var JobManager = module.exports = function JobManager(options){
    ...
    this.processShare = function(jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName, version, versionMask){
        ...
        var versionInt;
        if (version) {
            // BIP 310: Version should be sent as 4-byte hex string
            // Stratum sends it as big-endian hex, but we need the integer value
            versionInt = parseInt(version, 16);
            
            // Debug first share
            if (!this.debuggedVersion) {
                this.debuggedVersion = true;
                console.log('Version debug:');
                console.log('  Raw version hex:', version);
                console.log('  Parsed version int:', versionInt);
                console.log('  Job version:', job.rpcData.version);
                console.log('  Job version hex:', job.rpcData.version.toString(16));
            }
        } else {
