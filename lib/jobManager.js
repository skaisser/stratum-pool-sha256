...
var JobManager = module.exports = function JobManager(options){
    ...
    this.processShare = function(jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName, version){
        ...
        if (options.coin.asicboost) {
            // Validate version is within ASICBoost range
            if (versionInt < job.minVersion || versionInt > job.maxVersion) {
                console.log('Version out of range: ' + versionInt + ' not in [' + 
                           job.minVersion + ', ' + job.maxVersion + ']');
                return shareError([20, 'version out of range']);
            }
            // Check if rolled bits are within allowed mask
            var rolledBits = versionInt ^ job.rpcData.version;
            if ((rolledBits & ~job.versionMask) !== 0) {
                console.log('Invalid version rolling: rolled bits 0x' + rolledBits.toString(16) + 
                           ' not within mask 0x' + job.versionMask.toString(16));
                return shareError([20, 'invalid version rolling']);
            }
        }
